...
class ChattingRoomsScreen extends GetView<ChattingRoomsScreenController> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: const NotiAppBar(title: Text(" 채팅", style: TextStyle(fontSize: 20,
  ...
  Widget _buildChatroomList() {
    if (controller.chatrooms.isEmpty) {
      ...
      child: Column(
        ...
        children: [
          const Text("아직 아무도 친구가 되지 않았어요!", style: TextStyle(color: Color(0xff9f75d1),
              fontWeight: FontWeight.w600,
              fontSize: 18),),
          SizedBox(height: 10,),
          Text("친구 신청을 보내고", style: TextStyle(
              color: Color(0xff2d3a45).withOpacity(0.64),
              fontWeight: FontWeight.w400,
              fontSize: 14)),
          Text("새로운 친구를 만들어보세요", style: TextStyle(
              color: Color(0xff2d3a45).withOpacity(0.64),
              fontWeight: FontWeight.w400,
              fontSize: 14)),
          SizedBox(height: 36,),
        ...
        children: [
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                Row(children: [
                  Text("상대 이름", style: TextStyle(fontSize: 16,
                      fontWeight: FontWeight.w700,
                      color: Color(0xff2d3a45))),
                  SizedBox(width: 8),
                  Text("${chatroom.createdAt.toLocal().year}년 ${chatroom.createdAt.toLocal().month}월 ${chatroom.createdAt.toLocal().day}일", style: TextStyle(fontSize: 12,
                      fontWeight: FontWeight.w400,
                      color: Color(0xff9f75d1)))
                ],),
                SizedBox(height: 4),
                Text("채팅룸의 가장 최근 대화", style: TextStyle(fontSize: 14,
                    fontWeight: FontWeight.w400,
          ...
          Transform.rotate(
            ...
            child: PopupMenuButton(itemBuilder: (context) {
              return [PopupMenuItem<int>(value: 0, child: Text("퇴장")), PopupMenuItem<int>(value: 1, child: Text("읽음 처리")), PopupMenuItem<int>(value: 2, child: Text("차단"))];
            },
              onSelected: (value){
                if(value == 0) print("퇴장");
                else if (value == 1) print("읽음으로 처리");
